    ...
    <div class="max-w-7xl mx-auto px-6 py-16">
        <div class="grid md:grid-cols-4 gap-10 mb-12">
            <div class="md:col-span-2">
                ...
                @if($tenant->email)<a href="mailto:{{ $tenant->email }}" class="text-gray-400 text-sm hover:text-white transition-colors">{{ $tenant->email }}</a>@endif
            </div>
            <div>
                <p class="text-xs font-semibold tracking-widest uppercase text-gray-500 mb-4">Navigation</p>
                <ul class="space-y-2">
                    @foreach($navItems as $item)
                        <li><a href="{{ $item->url }}" class="text-gray-400 text-sm hover:text-white transition-colors">{{ $item->name }}</a></li>
                    @endforeach
                </ul>
            </div>
            <div>
                ...
                <a href="{{ route('donate') }}"
                   class="btn-primary inline-block px-6 py-3 {{ $config['button_radius'] ?? 'rounded-lg' }} text-sm font-semibold shadow-lg transition-all">
                    Get Started
                </a>
            </div>
        </div>
