...
<section style="background:{{ $bg }};" class="{{ $spacing }}">
    <div class="max-w-4xl mx-auto px-6 {{ $textAlign }}">
        @if(!empty($data['heading']))
            <h2 class="text-3xl md:text-4xl font-bold mb-6" data-animate="fade-up" style="color:{{ $textColor }};">
                {{ $data['heading'] }}
            </h2>
        @endif
        @if(!empty($data['content']))
            <div class="prose prose-lg max-w-none leading-relaxed" data-animate="fade-up" data-delay="120" style="color:{{ $subColor }};">
