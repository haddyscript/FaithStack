<header class="absolute top-0 inset-x-0 z-50">
    <div class="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
        ...
        <nav class="hidden md:flex items-center gap-8 text-sm font-medium text-white/80">
            @foreach($navItems as $item)
                <a href="{{ $item->url }}" class="hover:text-white transition-colors duration-150">{{ $item->name }}</a>
            @endforeach
            <a href="{{ route('donate') }}"
               class="border border-white/40 hover:border-white text-white px-5 py-2 {{ $config['button_radius'] ?? 'rounded-lg' }} text-sm font-semibold transition-all duration-150 backdrop-blur-sm">
                Get Started
            </a>
        </nav>
    ...
    <div x-show="mobileOpen"
         ...
         class="md:hidden px-6 py-4 space-y-3 bg-black/60 backdrop-blur-md">
        @foreach($navItems as $item)
            <a href="{{ $item->url }}" class="block text-sm font-medium text-white/80 hover:text-white">{{ $item->name }}</a>
        @endforeach
        <a href="{{ route('donate') }}"
           class="block text-center border border-white/40 text-white px-5 py-2.5 {{ $config['button_radius'] ?? 'rounded-lg' }} text-sm font-semibold">
            Get Started
        </a>
    </div>
</header>
